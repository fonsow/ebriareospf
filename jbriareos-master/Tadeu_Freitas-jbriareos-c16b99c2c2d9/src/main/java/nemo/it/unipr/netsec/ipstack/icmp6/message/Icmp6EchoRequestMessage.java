 ...
 */
public class Icmp6EchoRequestMessage extends Icmp6EchoMessage {

	/** ICMP type */
	public static final int TYPE=Icmp6Message.TYPE_Echo_Request;
	...
	public Icmp6EchoRequestMessage(Ip6Address src_addr, Ip6Address dst_addr, int id, int sqn, byte[] echo_data) {
	...
	public Icmp6EchoRequestMessage(Ip6Address src_addr, Ip6Address dst_addr, byte[] buf, int off, int len) {
		super(src_addr,dst_addr,buf,off,len);
		if (type!=TYPE) throw new RuntimeException("ICMP type missmatch ("+type+"): this is not an \"Echo Request\" ("+TYPE+") ICMP message");
	}	

	
	/** Creates a new ICMP Echo Request message.
	 * @param msg the ICMP message */
	public Icmp6EchoRequestMessage(Icmp6Message msg) {
		super(msg);
		if (type!=TYPE) throw new RuntimeException("ICMP type missmatch ("+type+"): this is not an \"Echo Request\" ("+TYPE+") ICMP message");
	}	
